...
public class AST_EXP_METHOD_CALL extends AST_EXP
{
	/***************/
	/*  var := exp */
	/***************/
	...
	/*  CONSTRUCTOR(S) */
	/*******************/
	public AST_EXP_METHOD_CALL(AST_VAR cls, String name,  AST_EXP_LIST args, int l)
	{
		/******************************/
		...
		/***************************************/
		if (args == null)System.out.format("====================== exp -> var DOT ID(%s) LPAREN RPAREN\n",name);
		if (args != null)System.out.format("====================== exp -> var DOT ID(%s) LPAREN expList RPAREN\n",name);
		/*******************************/
	...
	public void PrintMe()
	{
		...
		/********************************************/
		System.out.print("AST NODE METHOD CALL EXP\n");
	...
	public TYPE SemantMe(PrintWriter file_writer, int scope, String param, int fpOffset)
	{
		...
		if (!cls_type.isClass() ){
			System.out.format(">> ERROR(%d) field acces to non class type variable %s\n",line,name);	
		...
		while (tc != null){
			for ( TYPE_CLASS_METHODS l1 = tc.data_members.methods; l1 != null; l1 = l1.tail){
				if (l1.head.name.equals(name) ){
					if (l1.head != null && checkArgs(l1.head.params,typeArgs,file_writer) ){
						this.paramCnt = l1.head.paramCnt;
						return l1.head.returnType;
					}
				}
			}
			tc = tc.father;
		...
		System.out.format(">> ERROR(%d) there is no function %s in class\n",line,name);	
		file_writer.printf("ERROR(%d)\n",line);
		file_writer.close();
		System.exit(0);
		return null;
	...
	public TEMP IRme()
	{
		...
		IR.getInstance().Add_IRcommand( new IRcommand_Init_Func_call( this.paramCnt + 1,this.name) );
		...
		IR.getInstance().Add_IRcommand( new IRcommand_End_Method_call( dst,func_address,this.paramCnt + 1) );
